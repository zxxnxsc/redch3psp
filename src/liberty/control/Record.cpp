...
void CRecordDataForChase::ProcessControlCars(void)
{
    if (Status != STATE_PLAYBACK)
        return;
#ifdef RW_PSP
    // Chase playback overwrites every car transform from the recorded stream
    // later in this frame. Running full vehicle physics/collision for all 13
    // cars first is wasted work and made the 15 Hz recording visibly jump on
    // PSP. Keep the recorded matrices authoritative and skip that duplicate
    // simulation pass.
    return;
#endif
    for (int i = 0; i < NUM_CHASE_CARS; i++) {
...
void CRecordDataForChase::GiveUsACar(int32 mi, CVector pos, float angle, CAutomobile** ppCar, uint8 colour1, uint8 colour2)
{
...
#ifdef RW_PSP
		pspTraceBoot("W111 coche de persecucion no disponible; omitido");
#endif
		return;
	}
	...
	if(pCar == nil){
#ifdef RW_PSP
		pspTraceBoot("W112 coche de persecucion sin memoria; omitido");
#endif
		return;
	}
...
void CRecordDataForChase::StartChaseScene(float startTime)
{
	char filename[28];
#ifdef RW_PSP
	pspTraceBoot("S110 persecucion cinematica inicia");
	pspTraceMemory("antes persecucion");
#endif
	SetUpCarsForChaseScene();
#ifdef RW_PSP
	pspTraceBoot("S111 coches de persecucion preparados");
	pspTraceMemory("despues persecucion");
#endif
	Status = STATE_PLAYBACK;
	AnimTime = startTime;

 ...
 : CVehicle(CreatedBy)
{
	...
	if(colModel->lines == nil || colModel->numLines < 4){
		if(colModel->lines)
			RwFree(colModel->lines);
		colModel->lines = (CColLine*)RwMalloc(4*sizeof(CColLine));
		colModel->numLines = 4;
	}

    // Reuse the already validated model pointers. Looking the model up again
    // inside SetupSuspensionLines could return a transient/null entry while
    // the chase-scene cars were being streamed and caused the 10A crash.
    SetupSuspensionLines(mi, colModel);
...
void
CAutomobile::SetupSuspensionLines(CVehicleModelInfo *mi, CColModel *colModel)
{
    int i;
    CVector posn;
    if(mi == nil || colModel == nil || pHandling == nil ||
       colModel->lines == nil || colModel->numLines < 4)
        return;
	...
	for(i = 0; i < 4; i++){
#ifdef RW_PSP
		// Calling through the streamed wheel-frame hierarchy here corrupted the
		// MIPS stack for a few intro vehicles (the 9Y crash at 0x0895ab08).
		// Suspension only needs a stable local wheel position, so derive it from
		// the collision box on PSP. This also covers damaged/incomplete DFFs and
		// avoids four frame-tree walks for every constructed car.
		const float halfWidth = Max(0.35f,
			Max(Abs(colModel->boundingBox.min.x), Abs(colModel->boundingBox.max.x)) * 0.75f);
		posn.x = i < 2 ? -halfWidth : halfWidth;
		posn.y = (i == CARWHEEL_FRONT_LEFT || i == CARWHEEL_FRONT_RIGHT) ?
			colModel->boundingBox.max.y * 0.72f : colModel->boundingBox.min.y * 0.72f;
		posn.z = colModel->boundingBox.min.z + mi->m_wheelScale * 0.5f;
#else

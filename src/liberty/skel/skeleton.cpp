...
RsRwInitialize(void *displayID)
{
	...
	gPspBootStage = 10;
	pspTraceBoot("R10 antes de RwEngineInit");
#endif
...
#ifdef RW_PSP
	gPspBootStage = 11;
	pspTraceBoot("R11 RwEngineInit termino");
#endif
	...
	psInstallFileSystem();
#ifdef RW_PSP
	pspTraceBoot("R12 filesystem instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("R13 debug instalado");
#endif
...
#ifdef RW_PSP
	gPspBootStage = 12;
	pspTraceBoot("R14 plugins de GTA instalados");
#endif
...
#ifdef RW_PSP
	gPspBootStage = 13;
	pspTraceBoot("R15 dispositivos de entrada instalados");
#endif
...
#ifdef RW_PSP
	gPspBootStage = 14;
	pspTraceBoot("R16 RwEngineOpen termino");
#endif
...
#ifdef RW_PSP
	gPspBootStage = 15;
	pspTraceBoot("R17 modo de video seleccionado");
#endif
...
#ifdef RW_PSP
	gPspBootStage = 16;
	pspTraceBoot("R18 RwEngineStart termino");
	// On PSP the remaining desktop tail consists entirely of no-op shims:
	// RsRegisterImageLoader() only returns TRUE, psNativeTextureSupport() only
	// returns TRUE and both mipmapping setters only return TRUE.  Returning here
	// avoids crossing those legacy PC ABI wrappers after the GU and all plugins
	// are already fully started.  The 9I trace proved the crash occurred in that
	// redundant tail, between R18 and the caller's M5 marker.
	pspTraceBoot("R19 inicializacion RW PSP completa");
	POP_MEMID();
	return TRUE;
#endif

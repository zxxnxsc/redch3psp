...
{
#ifdef RW_PSP
	// PSP's device paths use "ms0:/" and forward slashes. GTA III data files
	// contain Windows separators, while the generic Linux casepath walker does
	// not understand the ms0: device prefix. Normalize directly; the Memory
	// Stick filesystem itself is case-insensitive.
	char pspPath[512];
	size_t len = strlen(filename);
	if(len >= sizeof(pspPath))
		return nil;
	for(size_t i = 0; i <= len; i++)
		pspPath[i] = filename[i] == '\\' ? '/' : filename[i];
	FILE *result = fopen(pspPath, mode);
	if(!result)

...
CTimeCycle::Initialise(void)
{
	...
	// Safe defaults prevent a missing or shortened file from producing a black
	// world. The original reader also required a newline after the final row and
	// would otherwise write beyond `line`, which caused the PSP crash at 08940894.
	for(w = 0; w < NUMWEATHERS; w++)
		for(h = 0; h < NUMHOURS; h++){
			m_nAmbientRed[h][w] = m_nAmbientGreen[h][w] = 110;
			m_nAmbientBlue[h][w] = 120;
			m_nDirectionalRed[h][w] = m_nDirectionalGreen[h][w] = 170;
			m_nDirectionalBlue[h][w] = 165;
			m_nSkyTopRed[h][w] = 70; m_nSkyTopGreen[h][w] = 120; m_nSkyTopBlue[h][w] = 180;
			m_nSkyBottomRed[h][w] = 135; m_nSkyBottomGreen[h][w] = 170; m_nSkyBottomBlue[h][w] = 205;
			m_nSunCoreRed[h][w] = m_nSunCoronaRed[h][w] = 255;
			m_nSunCoreGreen[h][w] = 240; m_nSunCoreBlue[h][w] = 210;
			m_nSunCoronaGreen[h][w] = 220; m_nSunCoronaBlue[h][w] = 170;
			m_fSunSize[h][w] = m_fSpriteSize[h][w] = m_fSpriteBrightness[h][w] = 1.0f;
			m_nShadowStrength[h][w] = 80;
			m_nLightShadowStrength[h][w] = m_nTreeShadowStrength[h][w] = 40;
			m_fFarClip[h][w] = 500.0f; m_fFogStart[h][w] = 350.0f;
			m_fLightsOnGroundBrightness[h][w] = 1.0f;
		}

	CFileMgr::SetDir("DATA");
	ssize_t length = CFileMgr::LoadFile("TIMECYC.DAT", work_buff, sizeof(work_buff)-1, "rb");
	CFileMgr::SetDir("");
	if(length < 0) length = 0;
	work_buff[length] = '\0';
#ifdef RW_PSP
	extern void pspTraceBoot(const char *message);
	pspTraceBoot(length > 0 ? "T50 TIMECYC.DAT abierto" : "T50 TIMECYC.DAT ausente, usando valores seguros");
#endif

	bi = 0;
	int slot = 0;
	while(bi < length && slot < NUMWEATHERS*NUMHOURS){
		li = 0;
		while(bi < length && work_buff[bi] != '\n'){
			if(li < (int)sizeof(line)-1)
				line[li++] = work_buff[bi];
			bi++;
		}
		if(bi < length && work_buff[bi] == '\n') bi++;
		line[li] = '\0';
		char *text = line;
		while(*text == ' ' || *text == '\t' || *text == '\r') text++;
		if(*text == '\0' || *text == '/' || *text == ';') continue;

		int fields = sscanf(text, "%d %d %d  %d %d %d  %d %d %d  %d %d %d "
...
#ifdef RW_PSP
	pspTraceBoot(slot == NUMWEATHERS*NUMHOURS ? "T51 96 ciclos horarios cargados" : "T51 ciclo horario parcial completado con valores seguros");
#endif

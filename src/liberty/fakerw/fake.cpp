...
// Plugin registration happens inside Engine::init(), before the Engine object
// itself exists.  Accessing engine->memfuncs here dereferenced a null pointer
// on PSP at the first plugin allocation.  The allocator table is static and is
// deliberately installed before module/plugin registration begins.
...
RwBool rwNativeTextureHackRead(RwStream *stream, RwTexture **tex, RwInt32 size)
{
	*tex = Texture::streamReadNative(stream);
	...
	if(*tex == nil)
		return false;
	if((*tex)->raster && (*tex)->raster->platform != rw::platform)
...
#elif defined(RW_PSP)
	// PSP has no desktop window/display object.  The generic skeleton passes a
	// null displayID, which the old Dreamcast fallback below dereferenced and
	// crashed before RenderWare could initialise.  The GU backend only needs a
	// valid EngineOpenParams object; DEVICEOPEN itself is intentionally a no-op.
	openParams.displayID = initParams ? initParams->displayID : nil;
#else

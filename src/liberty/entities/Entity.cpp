...
// A few GTA III IPL entries legitimately have no matching collision entry in
// the loaded COL archives.  The original PC executable happened to tolerate
// this through data/layout assumptions which are not safe on PSP.  Keep all
// bound queries valid and give those visual-only instances a small world box.
static CColModel *
GetEntityColModelSafe(int32 modelIndex)
{
	if(modelIndex >= 0 && modelIndex < MODELINFOSIZE){
		CBaseModelInfo *mi = CModelInfo::GetModelInfo(modelIndex);
		if(mi && mi->GetColModel())
			return mi->GetColModel();
	}
	return &CTempColModels::ms_colModelBBox;
}
...
CEntity::GetBoundRect(void)
{
	CRect rect;
	CVector v;
	CColModel *col = GetEntityColModelSafe(m_modelIndex);
...
{
	return GetMatrix() * GetEntityColModelSafe(m_modelIndex)->boundingSphere.center;
}
...
{
	TransformPoint(out, GetMatrix(), GetEntityColModelSafe(m_modelIndex)->boundingSphere.center);
}
#else
void
CEntity::GetBoundCentre(CVector &out)
{
	out = GetMatrix() * GetEntityColModelSafe(m_modelIndex)->boundingSphere.center;
}
#endif
...
{
	return GetEntityColModelSafe(m_modelIndex)->boundingSphere.radius;
}
...
CEntity::GetIsOnScreenComplex(void)
{
	...
	CColModel *colmodel = GetEntityColModelSafe(m_modelIndex);
...
CEntity::Add(void)
{
	...
	xstart = Clamp(xstart, 0, NUMSECTORS_X-1);
	xend   = Clamp(xend,   0, NUMSECTORS_X-1);
	xmid   = Clamp(xmid,   0, NUMSECTORS_X-1);
	ystart = Clamp(ystart, 0, NUMSECTORS_Y-1);
	yend   = Clamp(yend,   0, NUMSECTORS_Y-1);
	ymid   = Clamp(ymid,   0, NUMSECTORS_Y-1);
	assert(xstart >= 0);
...
{
	return -GetEntityColModelSafe(m_modelIndex)->boundingBox.min.z;
}

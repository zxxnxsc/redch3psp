...
{
	RwFrame *frame = m_clump && n >= 0 && n < 4 ?
		GetFrameFromId(m_clump, wheelIds[n]) : nil;
	if(frame){
	...
	// Some streamed PSP vehicle clumps can arrive without one of the four
	// named wheel frames. The original code dereferenced that missing frame and
	// crashed while the intro chase created its cars. Build a conservative
	// suspension position from the collision bounds so the vehicle remains
	// usable and the script can continue.
	CColModel *col = GetColModel();
	float halfWidth = 0.8f;
	float front = 1.2f;
	float rear = -1.2f;
	float wheelZ = -0.3f;
	if(col){
		halfWidth = Max(0.35f, Max(Abs(col->boundingBox.min.x), Abs(col->boundingBox.max.x)) * 0.75f);
		front = col->boundingBox.max.y * 0.72f;
		rear = col->boundingBox.min.y * 0.72f;
		wheelZ = col->boundingBox.min.z + m_wheelScale * 0.5f;
	}
	pos.x = n < 2 ? -halfWidth : halfWidth;
	pos.y = (n == 0 || n == 2) ? front : rear;
	pos.z = wheelZ;
#ifdef RW_PSP
	static int32 wheelFallbackReports = 0;
	if(wheelFallbackReports++ < 8)
		pspTraceBoot("W110 nodo de rueda ausente; suspension segura aplicada");
#endif
}

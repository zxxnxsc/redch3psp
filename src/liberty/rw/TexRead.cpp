...
namespace rw { extern int32 pspTextureMaxDimension; }
#endif
...
RwTexDictionaryGtaStreamRead(RwStream *stream)
{
	RwUInt32 size, version;
	RwInt32 numTextures;
	RwUInt16 dictionaryHeader[2];
	RwTexDictionary *texDict;
	RwTexture *tex;

	if(!RwStreamFindChunk(stream, rwID_STRUCT, &size, &version))
		return nil;
	if(size < sizeof(dictionaryHeader) ||
	   RwStreamRead(stream, dictionaryHeader, sizeof(dictionaryHeader)) != sizeof(dictionaryHeader))
		return nil;
	if(size > sizeof(dictionaryHeader))
		RwStreamSkip(stream, size - sizeof(dictionaryHeader));
	numTextures = dictionaryHeader[0];
	if(numTextures < 0 || numTextures > 4096)
		return nil;
#ifdef RW_PSP
    // Fixed PSP budget: 64px is enough for the 480x272 frontend, while large
    // world dictionaries use 32px. 10B raised frontend banks to 128px and
    // consumed the reserve needed by streamed gameplay TXDs.
    rw::pspTextureMaxDimension = numTextures >= 64 ? 32 : 64;
#endif
	...
	for(int32 textureIndex = 0; textureIndex < numTextures; textureIndex++){
		...
		if(gGameState != GS_PLAYING_GAME &&
		   (textureIndex == 0 || (textureIndex & 31) == 0)) {
			char trace[80];
			snprintf(trace, sizeof(trace), "D75 TXD textura %d de %d", textureIndex + 1, numTextures);
			pspTraceBoot(trace);
		}
#endif
...
#ifdef RW_PSP
			char trace[80];
			snprintf(trace, sizeof(trace), "E75 TXD fallo en textura %d de %d", textureIndex + 1, numTextures);
			pspTraceBoot(trace);
#endif
...
#ifdef RW_PSP
	if(gGameState != GS_PLAYING_GAME)
		pspTraceBoot("D76 diccionario TXD decodificado");
#endif

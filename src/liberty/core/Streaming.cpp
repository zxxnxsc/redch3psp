...
CStreaming::LoadCdDirectory(const char *dirname, int32 n)
{
	...
	if(fd <= 0){
#ifdef RW_PSP
		pspTraceBoot("E91 GTA3.DIR no pudo abrirse");
#endif
		return;
	}
...
#ifdef RW_PSP
		// PSP IO is synchronous and its heap cannot afford keeping half-decoded
		// TXDs alive. Decode once, install once, and never enter the PC split path.
...
CStreaming::LoadAllRequestedModels(bool priority)
{
	...
	while(ms_endRequestedList.m_prev != &ms_startRequestedList){
		...
		if(ms_aInfoForModel[streamId].GetCdPosnAndSize(posn, size)){
			...
			int32 syncStatus = status == STREAM_SUCCESS ? CdStreamSync(0) : STREAM_ERROR;
			if(status != STREAM_SUCCESS || syncStatus != STREAM_NONE){
				char trace[128];
				snprintf(trace, sizeof(trace), "E92 lectura IMG fallo id=%d pos=%u size=%u", streamId, posn, size);
				pspTraceBoot(trace);
...
#ifdef RW_PSP
			static int32 pspModelsLoaded = 0;
			if((++pspModelsLoaded & 7) == 1){
				char trace[96];
				snprintf(trace, sizeof(trace), "D93 streaming modelo %d id=%d", pspModelsLoaded, streamId);
				pspTraceBoot(trace);
			}
#endif

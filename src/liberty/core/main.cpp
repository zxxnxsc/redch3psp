...
static float
EffectiveFarClip(void)
{
#ifdef RW_PSP
	return Min(CTimeCycle::GetFarClip(), 150.0f);
#else
	return CTimeCycle::GetFarClip();
#endif
}

static float
EffectiveFogStart(void)
{
#ifdef RW_PSP
	return Min(CTimeCycle::GetFogStart(), 110.0f);
#else
	return CTimeCycle::GetFogStart();
#endif
}
...
PluginAttach(void)
{
	...
	pspTraceBoot("P20 PluginAttach entro");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P21 World instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P22 Skin instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P23 HAnim instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P24 NodeName instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P25 Visibility instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P26 AnimBlend instalado");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("P27 MatFX instalado");
#endif
#ifdef ANISOTROPIC_FILTERING
...
{
#ifdef RW_PSP
	extern volatile int gPspBootStage;
	gPspBootStage = 20;
#endif
	if (RsRwInitialize(param))
	{
		...
		DebugMenuPopulate();
#endif // !DEBUGMENU
		RwBool result = CGame::InitialiseRenderWare();
#ifdef RW_PSP
		gPspBootStage = result ? 22 : 23;
#endif
		return result;
	}
...
{
#ifdef RW_PSP
	// PC loading screens depend on loose TXD/mainsc*.txd files which are not
	// part of a normal GTA III installation. Never block boot waiting for them:
	// show a lightweight PSP-native progress frame instead.
	(void)str2;
	(void)splashscreen;
	if(DoRWStuffStartOfFrame(12, 24, 38, 12, 24, 38, 255)) {
		CSprite2d::SetRecipNearClip();
		CSprite2d::InitPerFrame();
		DefinedState();
		if(str1) NumberOfChunksLoaded += 1.0f;
		float progress = NumberOfChunksLoaded / TOTALNUMCHUNKS;
		if(progress < 0.03f) progress = 0.03f;
		if(progress > 1.0f) progress = 1.0f;
		CSprite2d::DrawRect(CRect(36.0f, SCREEN_HEIGHT - 24.0f,
		                           SCREEN_WIDTH - 36.0f, SCREEN_HEIGHT - 16.0f),
		                    CRGBA(35, 52, 70, 255));
		CSprite2d::DrawRect(CRect(36.0f, SCREEN_HEIGHT - 24.0f,
		                           36.0f + (SCREEN_WIDTH - 72.0f)*progress,
		                           SCREEN_HEIGHT - 16.0f),
		                    CRGBA(238, 125, 34, 255));
		DoRWStuffEndOfFrame();
	}
	return;
#endif
...
#ifdef RW_PSP
	// Keep gameplay-critical feedback while dropping the heaviest decorative
	// passes. This profile targets the 64 MiB PSP-2000, not desktop PPSSPP.
	CCoronas::Render();
	CParticle::Render();
	CPacManPickups::Render();
	CWeaponEffects::Render();
...
Idle(void *arg)
{
	   ...
	   TheCamera.GetScreenFadeStatus() != FADE_2)
	{
		...
		RwCameraSetFarClipPlane(Scene.camera, EffectiveFarClip());
		RwCameraSetFogDistance(Scene.camera, EffectiveFogStart());
#endif

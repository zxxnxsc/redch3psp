...
{
	if(fd <= 0 || fd >= NUMFILES || myfiles[fd].file == nil || buf == nil)
		return 0;
...
CFileMgr::LoadFile(const char *file, uint8 *buf, int maxlen, const char *mode)
{
	...
	len = 0;
	while(len < maxlen-1){
		ssize_t remaining = maxlen-1-len;
		ssize_t chunk = remaining < 0x4000 ? remaining : 0x4000;
		n = myfread(buf + len, 1, chunk, fd);
		len += n;
		if(n < chunk)
			break;
	}

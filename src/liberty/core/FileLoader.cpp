...
GetFilename(const char *filename)
{
	...
	char *forward = strrchr((char*)filename, '/');
	if(forward && (!s || forward > s)) s = forward;
...
CFileLoader::LoadLevel(const char *filename)
{
	...
	if(fd <= 0){
#ifdef RW_PSP
		pspTraceBoot("E63 archivo DAT principal no pudo abrirse");
#endif
		RwTexDictionarySetCurrent(savedTxd);
		return;
	}
	...
	{
		char trace[160];
		snprintf(trace, sizeof(trace), "D70 DAT abierto: %s", filename);
		pspTraceBoot(trace);
	}
#endif
	...
	for(line = LoadLine(fd); line; line = LoadLine(fd)){
		...
		if(strncmp(line, "IMAGEPATH", 9) == 0){
			...
			{
				char trace[160];
				snprintf(trace, sizeof(trace), "D71 TXD inicia: %s", txdname);
				pspTraceBoot(trace);
			}
#endif
...
#ifdef RW_PSP
			pspTraceBoot("D72 TXD integrado");
#endif
			POP_MEMID();
...
#ifdef RW_PSP
			pspTraceBoot("D73 COLFILE procesado");
#endif
...
CFileLoader::LoadCollisionFile(const char *filename)
{
	...
	while(CFileMgr::Read(fd, (char*)&header, sizeof(header))){
		if(header.ident != 'CLOC' || header.size < 24 || header.size > sizeof(work_buff))
			break;
		if(CFileMgr::Read(fd, (char*)work_buff, header.size) != header.size)
			break;
...
CFileLoader::LoadScene(const char *filename)
{
	...
	char pathTypeStr[20];
	char sceneName[128];
	strncpy(sceneName, filename, sizeof(sceneName)-1);
	...
	fd = CFileMgr::OpenFile(sceneName, "rb");
	if(fd <= 0){
#ifdef RW_PSP
		char trace[128];
		snprintf(trace, sizeof(trace), "E80 IPL no pudo abrirse: %s", sceneName);
		pspTraceBoot(trace);
#endif
		return;
	}
	...
	{
		char trace[128];
		snprintf(trace, sizeof(trace), "D80 IPL inicia: %s", sceneName);
		pspTraceBoot(trace);
	}
#endif
	for(line = CFileLoader::LoadLine(fd); line; line = CFileLoader::LoadLine(fd)){
	...
	{
		char trace[128];
		snprintf(trace, sizeof(trace), "D82 IPL completo: %s", sceneName);
		pspTraceBoot(trace);
	}
#endif
}
...
CFileLoader::LoadObjectInstance(const char *line)
{
	...
	RwMatrix *xform;
	CEntity *entity;
	if(sscanf(line, "%d %23s %f %f %f %f %f %f %f %f %f %f",
	          &id, name,
	...
	if(mi == nil || !mi->IsSimple())
		return;
	if(mi->GetColModel() == nil){
		// Shared, non-owned fallback: preserve the visual IPL instance while
		// preventing null bound/collision dereferences on PSP.
		mi->SetColModel(&CTempColModels::ms_colModelBBox, false);
#ifdef RW_PSP
		static int32 missingColReports = 0;
		if(missingColReports++ < 24){
			char trace[160];
			snprintf(trace, sizeof(trace), "W81 COL respaldo id=%d modelo=%s", id, mi->GetModelName());
			pspTraceBoot(trace);
		}
#endif
	}

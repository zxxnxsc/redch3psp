...
#ifdef RW_PSP
		// The PSP build reuses the Dreamcast semantic control layer, while the
		// frontend and several shared systems are compiled against the native
		// PlayStation button names. Mirror every physical button into both
		// representations so D-pad navigation, CROSS accept, CIRCLE back and
		// START pause all observe the same sampled frame.
		NewState.Cross          = NewState.A ? 255 : 0;
		NewState.Circle         = NewState.B ? 255 : 0;
		NewState.Square         = NewState.X ? 255 : 0;
		NewState.Triangle       = NewState.Y ? 255 : 0;
		NewState.LeftShoulder1  = NewState.LeftTrigger ? 255 : 0;
		NewState.RightShoulder1 = NewState.RightTrigger ? 255 : 0;
#endif
...
#ifdef RW_PSP
	return NewState.RightTrigger ? 255 : 0;
#endif
...
#ifdef RW_PSP
	return !!(NewState.LeftTrigger && !OldState.LeftTrigger);
#endif

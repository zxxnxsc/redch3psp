...
CGame::InitialiseRenderWare(void)
{
	...
	pspTraceBoot("G20 InitialiseRenderWare entro");
#endif
#ifdef USE_TEXTURE_POOL
...
#ifdef RW_PSP
	pspTraceBoot("G21 TXD store inicializado");
#endif
	CVisibilityPlugins::Initialise();	// in plugin attach on ps2
#ifdef RW_PSP
	pspTraceBoot("G22 listas de visibilidad inicializadas");
#endif
#endif
...
#ifdef RW_PSP
	pspTraceBoot(Scene.camera ? "G23 camara creada" : "E23 camara nula");
#endif
...
#ifdef RW_PSP
	RwCameraSetFarClipPlane(Scene.camera, 150.0f);
#else
...
#ifdef RW_PSP
	pspTraceBoot("G24 camara dimensionada");
#endif
...
#ifdef RW_PSP
	pspTraceBoot(Scene.world ? "G25 mundo creado" : "E25 mundo nulo");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("G26 camara y luces agregadas");
#endif

#if GTA_VERSION > GTA3_PS2_160
	// RtCharset is a desktop-only debug overlay, not the GTA HUD. The PSP
	// raster path does not need it in the release homebrew.
#ifndef RW_PSP
	CreateDebugFont();	// in GameInit on PS2
#else
	pspTraceBoot("G27 fuente debug PC omitida en PSP");
#endif
#else
	...
	CFont::Initialise();
#ifdef RW_PSP
	pspTraceBoot("G28 fuentes GTA inicializadas");
#endif
	CHud::Initialise();
#ifdef RW_PSP
	pspTraceBoot("G29 HUD inicializado");
#endif
	POP_MEMID();
	// TODO: define
	CPlayerSkin::Initialise();
#ifdef RW_PSP
	pspTraceBoot("G30 skin del jugador inicializado");
#endif
#endif
...
#ifdef RW_PSP
	pspTraceBoot("G31 InitialiseRenderWare completo");
#endif
	return (true);
}
...
bool CGame::InitialiseOnceAfterRW(void)
{
...
#if GTA_VERSION > GTA3_PS2_160
	#ifdef RW_PSP
	pspTraceBoot("O40 InitialiseOnceAfterRW entro");
	#endif
	TheText.Load();
	#ifdef RW_PSP
	pspTraceBoot("O41 textos GXT cargados");
	#endif
	DMAudio.Initialise();	// before TheGame() on PS2
	#ifdef RW_PSP
	pspTraceBoot("O42 audio logico inicializado");
	#endif
	CTimer::Initialise();
	#ifdef RW_PSP
	pspTraceBoot("O43 temporizador inicializado");
	#endif
	CTempColModels::Initialise();
	#ifdef RW_PSP
	pspTraceBoot("O44 colisiones temporales inicializadas");
	#endif
	mod_HandlingManager.Initialise();
	#ifdef RW_PSP
	pspTraceBoot("O45 handling cargado");
	#endif
	CSurfaceTable::Initialise("DATA\\SURFACE.DAT");
	#ifdef RW_PSP
	pspTraceBoot("O46 superficies cargadas");
	#endif
	CPedStats::Initialise();
	#ifdef RW_PSP
	pspTraceBoot("O47 estadisticas de peatones cargadas");
	#endif
	CTimeCycle::Initialise();
	#ifdef RW_PSP
	pspTraceBoot("O48 ciclo horario cargado");
	#endif
	...
	#ifdef RW_PSP
	pspTraceBoot("O49 preferencias y skin aplicadas");
	#endif
#endif
	#ifdef RW_PSP
	pspTraceBoot("O50 InitialiseOnceAfterRW completo");
	#endif
	return true;
}
...
#endif
{
	...
	pspTraceBoot("L60 CGame Initialise entro");
	pspTraceMemory("inicio juego");
#endif
#ifdef GTA_PS2
	...
	POP_MEMID();
#ifdef RW_PSP
	pspTraceBoot("L61 texturas genericas y particulas listas");
#endif
	...
	POP_MEMID();
#ifdef RW_PSP
	pspTraceBoot("L62 nucleo y mundo inicializados");
#endif
	...
	POP_MEMID();
#ifdef RW_PSP
	pspTraceBoot("L62A particulas DATA inicializadas");
#endif
	...
	POP_MEMID();
#ifdef RW_PSP
	pspTraceBoot("L62B gestores de animacion listos");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("L62C informacion de modelos lista");
#endif
	...
	bool gtaImgOpened = CdStreamAddImage("MODELS\\GTA3.IMG");
#ifdef RW_PSP
	pspTraceBoot(gtaImgOpened ? "L62D GTA3.IMG abierto" : "E62D GTA3.IMG no pudo abrirse");
	pspTraceBoot("L62E antes de DEFAULT.DAT");
#endif

	CFileLoader::LoadLevel("DATA\\DEFAULT.DAT");
#ifdef RW_PSP
	pspTraceBoot("L62F DEFAULT.DAT procesado");
	pspTraceBoot("L62G antes de GTA3.DAT");
#endif
	CFileLoader::LoadLevel(datFile);
#ifdef RW_PSP
	pspTraceBoot("L62H GTA3.DAT procesado");
#endif
#ifdef RW_PSP
	pspTraceBoot("L63 DEFAULT.DAT y GTA3.DAT cargados");
#endif
#else
...
#ifdef RW_PSP
	pspTraceBoot("L64 paths preparados");
#endif
#if GTA_VERSION > GTA3_PS2_160
	...
	TestModelIndices();
#ifdef RW_PSP
	pspTraceBoot("L64A jugador y modelos verificados");
#endif
#endif

	LoadingScreen("Loading the Game", "Setup water", nil);
#ifdef RW_PSP
	pspTraceBoot("L64B agua inicia");
#endif
	CWaterLevel::Initialise("DATA\\WATER.DAT");
#ifdef RW_PSP
	pspTraceBoot("L64C agua lista");
#endif
#if GTA_VERSION <= GTA3_PS2_160
	...
	TheConsole.Init();
#ifdef RW_PSP
	pspTraceBoot("L64D consola lista");
#endif
#endif
	CDraw::SetFOV(120.0f);
#ifdef RW_PSP
	CDraw::ms_fLODDistance = 150.0f;
#else
...
#ifdef RW_PSP
	pspTraceBoot("L64E streaming inicia");
#endif
	CStreaming::Init();
#ifdef RW_PSP
	pspTraceBoot("L64F directorio y buffers streaming listos");
	// PED.IFP is permanent engine data. Decode it while the PSP heap still has
	// a large contiguous block; initial vehicles/TXDs are loaded afterwards.
	pspTraceBoot("L64F1 precarga de animaciones inicia");
	CAnimManager::LoadAnimFile("ANIM\\PED.IFP");
	pspTraceBoot("L64F2 precarga de animaciones termina");
#endif
	CStreaming::LoadInitialVehicles();
#ifdef RW_PSP
	pspTraceBoot("L64G vehiculos iniciales solicitados");
#endif
	CStreaming::LoadInitialPeds();
#ifdef RW_PSP
	pspTraceBoot("L64H peatones iniciales solicitados");
#endif
	CStreaming::RequestBigBuildings(LEVEL_GENERIC);
#ifdef RW_PSP
	pspTraceBoot("L64I edificios LOD solicitados");
#endif
	CStreaming::LoadAllRequestedModels(false);
#ifdef RW_PSP
	pspTraceBoot("L65 streaming inicial completo");
	pspTraceMemory("post streaming");
#endif
#if GTA_VERSION > GTA3_PS2_160
	...
	POP_MEMID();
#ifdef RW_PSP
	pspTraceBoot("L66 animaciones cargadas");
#endif
...
//	CWorld::Players[0].LoadPlayerSkin();	// TODO: use a define for this
	...
	POP_MEMID();
#ifdef RW_PSP
	pspTraceBoot("L67 scripts y pandillas inicializados");
#endif
	...
	CReplay::Init();
#ifdef RW_PSP
	pspTraceBoot("L68 sistemas de mundo inicializados");
#endif
...
#ifdef RW_PSP
	pspTraceBoot("L69 CGame Initialise completo");
	pspTraceMemory("juego listo");
#endif
#ifdef RW_DC
...
#if defined(DEBUGMENU) && !defined(RW_PSP)
	DebugMenuProcess();
#endif
	CCutsceneMgr::Update();
